...
def plot_risk_calculation_before_after(
    ...
    Args:
        ...
        output_json_file: The path to save the output file as a JSON file. If None, the output is not saved (default is None). Skipped if output_json_response is True.
        output_png_file: The path to save the output plot as a PNG file. If None, the plot is not saved (default is None). Skipped if output_json_response is True.
        output_json_response: Whether to return the output as a JSON response (default is False). Overrides output_json_file and output_png_file if True.
    ...
    Returns:
        dict[str, float] | None: If output_json_response is True, returns a dictionary containing the statistics and simulation results. Otherwise, returns None.
    """
    # Input validation
    ...
    # Return JSON response if requested, excluding simulation results for faster response
    if output_json_response:
        return {k: v for k, v in return_data.items() if k != "simulation_results"}
